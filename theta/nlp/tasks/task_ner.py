        ...
        def seg_encode(text, tags):
            ...
            if tags is not None:
                ...
                for tag in tags:
                    ...
                    #  )
                    start_ids[s + 1] = self.label2id[c]
                    end_ids[e + 1] = self.label2id[c]
...
# ------------------------------ TaskModel ------------------------------
class NerModel(TaskModel):
    """
    任务专属模型定义
    """
    def __init__(self, task_args, ner_labels):
        super(NerModel, self).__init__(**task_args.to_dict())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        batch_pred_tags = self.model.decode_ents(start_logits, end_logits)
    ...
    def show_val_results(self, category_p_r_f1):
        ...
        c_p_r_f1 = sorted(category_p_r_f1.items(),
                          ...
                          reverse=True)
        max_key_len = max([len(c.encode('gbk')) + 1 for c, _ in c_p_r_f1])
        #  max_key_len = 30

        title = f"{' '*max_key_len}   acc    recall f1    "
        title += "   tp/   fp/   tp+fn "
        ...
        for c, ((p, r, f1), (tp, fp, fn)) in c_p_r_f1:
            ...
            info = f"{disp_key} | {p:.4f} {r:.4f} {f1:.4f}"
            info += f" {tp:4d}/ {fp:4d}/ {tp+fn:4d}"
            logger.info(info)
            total_tp += tp
            total_fp += fp
            total_fn += fn
        ...
        info = f" Micro{' '*(max_key_len - 6)} | {micro_acc:.4f} {micro_recall:.4f} {micro_f1:.4f}"  #" - loss: {results['loss']:.4f}"
        info += f" {total_tp:4d}/ {total_fp:4d}/ {total_fn:4d}"
        logger.info(info)
        ...
        info = f" Macro{' '*(max_key_len - 6)} | {macro_acc:.4f} {macro_recall:.4f} {macro_f1:.4f}"  #" - loss: {results['loss']:.4f}"
        logger.info(info)
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred_tags = self.model.decode_ents(start_logits, end_logits)
