...
def collate_fn(batch):
    ...
    batch_labels = torch.tensor(
        sequence_padding(batch_labels, seq_dims=4), dtype=torch.long, device=device
    )
    ...
    def scores_to_mentions(scores):
        ...
        for i, (score, sent_text) in enumerate(zip(scores, sentences)):
            ...
            for r_l, start, end in zip(*np.where(score.cpu() > threshold)):
                ...
                if (
                    r in relations_id2label
                    and l in entities_id2label
                    and start >= 1
            ...
            for relation_label, e_list in R.items():
                ...
                text_tags.append({"relation": relation_label, "entities": entity_tags})

            tags_list.append({"text": sent_text, "tags": text_tags})

        return tags_list

    tags_list = scores_to_mentions(scores)

    full_tags = tags_list[0]
