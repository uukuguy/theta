...
def encode_text(text, tags, task_labels, max_length, tokenizer):
    ...
    # FIXME
    head_tail_labels = [set() for _ in range(len(relations_label2id))]
    tail_head_labels = [set() for _ in range(len(relations_label2id))]
    ...
    for sh, st, p, oh, ot in spoes:
        ...
        # FIXME
        head_tail_labels[p].add((sh, ot))
        tail_head_labels[p].add((oh, st))
    for label in entity_labels + head_labels + tail_labels + head_tail_labels + tail_head_labels:
    ...
    # [关系个数, 该关系下subject/object配对数, subject/object终点]
    tail_head_labels = sequence_padding([list(l) for l in tail_head_labels])
...
def encode_sentences(text_tags_list, task_labels, max_length, tokenizer):
    ...
    head_tail_labels_list, tail_head_labels_list = [], []
    for text, tags in tqdm(text_tags_list):
        ...
        # FIXME
        head_tail_labels_list.append(head_tail_labels)
        tail_head_labels_list.append(tail_head_labels)

    return (
        (tokens_list, mappings_list),
        (token_ids_list, segment_ids_list),
        (entity_labels_list, head_labels_list, tail_labels_list, head_tail_labels_list, tail_head_labels_list),
    )
...
class TaskDataset(Dataset):
    def __init__(self, args, data_generator, tokenizer):
        ...
        self.entity_labels_list, self.head_labels_list, self.tail_labels_list, self.head_tail_labels_list, self.tail_head_labels_list = (
            entity_labels_list,
            head_labels_list,
            tail_labels_list,
            head_tail_labels_list,
            tail_head_labels_list,
        )
    ...
    def __getitem__(self, i):
        ...
        # FIXME
        head_tail_labels = self.head_tail_labels_list[i]
        tail_head_labels = self.tail_head_labels_list[i]
