...
def run_training(
    ...
    with trange(max_episodes, desc=f"Episode", ncols=160) as pbar:
        for ep in pbar:
            ...
            evaluator = Evaluator(model, val_dataloader, categories_id2label, best_f1=last_best_f1)
            model.fit(
                train_dataloader,
...
def run_predicting(args, ner_labels, test_data, best_model_file, bert_model_path, tokenizer):
    ...
    for idx, full_text, true_tags in test_data:

        token_ids, encoded_labels = encode_text(full_text, true_tags, categories_label2id, args.max_length, tokenizer)
        batch_token_ids = [token_ids]
        batch_token_ids = torch.tensor(sequence_padding(batch_token_ids), dtype=torch.long, device=device)

        full_tags, sent_tags_list = predict_text(
            args, model, full_text, tokenizer, ner_labels, repeat=args.repeat, threshold=0
        )
