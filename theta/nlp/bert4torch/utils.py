...
def text_augmentation(
    texts, noise_dict=None, noise_len=0, noise_p=0.0, skip_words=None, strategy='random', allow_dup=True
):
    '''简单的EDA策略, 增删改
    ...
    def search(pattern, sequence, keep_last=True):
        ...
        for i in range(len(sequence)):
            if sequence[i:i + n] == pattern:
                pattern_idx_set = pattern_idx_set.union(set(range(i, i + n))) if keep_last else pattern_idx_set.union(
                    set(range(i, i + n - 1))
                )
    ...
    def update(self, current, values=None):
        ...
        if self.verbose == 1:
            ...
            if self.target is not None and current < self.target:
                ...
                if passed > 3600:
                    passed_format = ('%d:%02d:%02d' % (passed // 3600, (passed % 3600) // 60, passed % 60))
                elif passed > 60:
                    passed_format = '%d:%02d' % (passed // 60, passed % 60)
                else:
                    passed_format = '%ds' % passed

                #  info = ' - ETA: %s' % eta_format
                info = ' - %s<%s' % (passed_format, eta_format)
            else:
                if time_per_unit >= 1:
...
class ProgbarLogger(Callback):
    ...
    def add_metrics(self, metrics, add_position=None):
        ...
        self.params['metrics'
                    ] = self.params['metrics'][:add_position] + add_metrics + self.params['metrics'][add_position:]
    ...
    def forward_(self, train_X, new_embed):
        ...
        if isinstance(train_X, (tuple, list)):
            ...
            adv_output = self.model.forward(
                *new_train_X
            ) if self.model.forward.__code__.co_argcount >= 3 else self.model.forward(new_train_X)
