...
def debug_show_list(list_data):
    st.write([f"{data}" for data in list_data])
...
train_json_data = get_train_json_data()
st.write(f"训练数据集 (共{len(train_json_data)}条)")
train_json_data[:10]
