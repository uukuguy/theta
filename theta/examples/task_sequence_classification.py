...
TASK_NAME = "task_sequence_classification"
...
from theta.nlp.sequence_classification.tagging import(
    TaskLabels,
    TaskTag,
    TaggedData
)
...
from theta.nlp.sequence_classification.dataset import TaskDataset
from theta.nlp.sequence_classification.runner import (
    run_training,
    run_evaluating,
    run_predicting,
)
...
args = DictObject(
    **dict(
        ...
        num_classes=len(task_labels.labels),
        threshold=0,
        # 是否将文本划分为多个短句
        do_split=False,
        # Training
...
def tag_text(idx, line):
    ...
    # print("idx:", idx, "text_a:", text_a, "text_b:", text_b, "tags:", tags)
    return TaskTag(idx, text_a, text_b, label, None)
...
def build_final_result(d, real_tags):
    ...
    """
    idx, text_a, text_b, true_label, _ = d
...
def predict_test_file(test_file, best_model_file, results_file="results.json"):
    ...
    predictions = run_predicting(
        args, task_labels, test_data, best_model_file, bert_model_path, tokenizer
    )
