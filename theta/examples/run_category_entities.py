...
from theta.utils import init_random, init_cuda, init_labels
from theta.modeling.trainer import get_default_optimizer_parameters
from theta.modeling.ner import InputExample, examples_to_dataset, load_examples_from_bios_file, NerTrainer, get_entities
from theta.modeling.ner import load_pretrained_tokenizer, load_pretrained_model
...
# -------------------- Data --------------------
train_base_file = "../../../CCKS2020/event_element/data/event_element_train_data_label.txt"
test_base_file = "../../../CCKS2020/event_element/data/event_element_dev_data.txt"
...
def seg_generator(iterables, seg_len, seg_offset=0):
    if seg_len <= 0:
        yield iterables
    else:
...
def load_test_examples(test_base_file):
    with open(test_base_file, 'r') as fr:
        ...
        for line in tqdm(lines, desc=f"test"):
            ...
            for seg_words, in seg_generator((words, ), seg_len, seg_offset):
...
def merge_multi(args):
    ...
    for result_file in files:
        ...
        for doc_id, events in single_events.items():
            ...
            if len(t_events) == 0:
                final_events = [e for e in s_events if e['event_type'] == master_s_event_type]
            else:
...
def main(args):
    ...
    if args.do_train:
        ...
        trainer.train(args, train_dataset, eval_dataset)
    ...
    if args.do_eval:
        ...
        trainer.evaluate(args, model, eval_dataset)
    ...
    if args.do_predict:
        ...
        trainer.predict(args, model, test_dataset)
