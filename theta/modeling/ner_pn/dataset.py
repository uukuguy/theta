...
class InputFeature(object):
    ...
    def __init__(self, input_ids, input_mask, input_len, segment_ids, labels,
                 subjects):
...
def encode_examples(examples, label2id, tokenizer, max_seq_length):
    ...
    def encode_subjects(subjects, num_tokens):
        ...
        #  labels = [label2id['[unused1]']] * num_tokens
        #  labels = [[0, 0] * num_labels] * num_tokens
        ...
        if subjects:
            for subject in subjects:
                ...
                #  labels[start + 1][label2id[label]-1][0] = 1
                #  labels[end + 1][label2id[label]-1][1] = 1
                labels[start + 1][(label2id[label] - 1) * 2] = 1
                labels[end + 1][(label2id[label] - 1) * 2 + 1] = 1
    ...
    for input_ids, attention_mask, token_type_ids, subjects in zip(
        ...
        subjects_id, labels = encode_subjects(subjects, num_tokens)
    ...
    all_features = [
        InputFeature(input_ids=input_ids,
                     ...
                     input_len=input_len)
        for input_ids, attention_mask, token_type_ids, labels, subjects_ids,
        input_len in zip(all_input_ids, all_attention_mask, all_token_type_ids,
                         all_labels, all_subjects_ids, all_input_lens)
    ]
    return (all_features, {
        'input_ids':
